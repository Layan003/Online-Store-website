    ...
    <section className="cart-section flex justify-center gap-5 ">
      <div className="w-6/12 bg-blue-200 shadow-lg bg-white rounded-lg cart-container">
        ...
        {cartItems ? (cartItems.map((item) => (
          <div className=" w-auto rounded-lg " key={item.product.id}>
          <div className="flex w-auto p-4 cart-item" >
            <div className="w-[170px] h-[150px] align-center cart-img-container">
              <img src={`http://localhost:8000${item.product.image}`} className="cart-img" />
            </div>
            <div className="w-[70%] px-4 ">
              <div className="flex-col ">
                ...
                <div className="flex items-center mb-7">
                  ...
                  {item.product.stock_quantity >0 ? (<p className="text-green-500 font-semibold inline-block">
                    In Stock
                  </p>) : (<p className="text-red-500 font-semibold inline-block">
                    Out of Stock
                  </p>)}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center shadow-sm bg-gray-100 w-fit h-7 border border-gray-400 rounded-lg">
                    <div onClick={() => changItemQuantity(item.product.id)} className="count-down px-2">-</div>
                    <div className="px-2">{item.quantity}</div>
                    <div onClick={() => addCartItem(item.product.id)} className="count-up px-2">+</div>
                  </div>
                  ...
                  <div className="flex items-center">
                    ...
                    <p onClick={() => removeCarItem(item.product.id)} className="text-sm text-gray-600">Remove</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* order info */}
      {(cartItems && isAuthenticated) && ( <div className="w-3/12 bg-white h-fit p-4 shadow-lg cart-info">
        ...
        <div className="flex justify-between my-3 mx-2">
          ...
          <p className="font-semibold text-lg">${(parseFloat(totalPrice) + delivery).toFixed(2)}</p>
        </div>
